...
class Roavio_Breadcrumb
{
	...
	public static function breadcrumb_init()
	{
		...
		$home_title = Roavio_Helper::get_option('breadcrumb_home_title', esc_html__('Home', 'roavio'));
		$home       = home_url();
		...
		$html_markup .= '<div class="roavio-breadcrumb wt-breadcrumb-body wt-breadcrumb-body details-body">';
		...
		if (is_home() && !is_paged() || is_front_page() && !is_paged()) {
			...
			if (is_archive()) {
				if (is_category()) {
					...
					$html_markup .= $currentBefore . esc_html__('Archive by category &#39;', 'roavio');
					$html_markup .= single_cat_title('', false);
					$html_markup .= esc_html__('&#39;', 'roavio') . $currentAfter;
				} elseif (is_tag()) {
					$html_markup .= $currentBefore . esc_html__('Posts tagged &#39;', 'roavio');
					$html_markup .= single_tag_title('', false);
					$html_markup .= esc_html__('&#39;', 'roavio') . $currentAfter;
				} elseif (is_tax()) {
					$html_markup .= $currentBefore . esc_html__('Archive by &#39;', 'roavio');
					$html_markup .= single_cat_title('', false);
					$html_markup .= esc_html__('&#39;', 'roavio') . $currentAfter;
				} elseif (is_day()) {
					...
					$html_markup .= $currentBefore . esc_html__('Articles by ', 'roavio') . $userdata->display_name . $currentAfter;
				...
				if ('roavio_portfolio' == get_post_type()) {
					$parent_page = Roavio_Helper::get_option('portfolio_parent_page', 'archive_page');
					$custom_page = Roavio_Helper::get_option('portfolio_custom_page', '');
				...
				$html_markup .= $currentBefore . esc_html__('Search results for &#39;', 'roavio') . get_search_query() . esc_html__('&#39;', 'roavio') . $currentAfter;
			} elseif (is_404()) {
				$html_markup .= $currentBefore . esc_html__('Error 404', 'roavio') . $currentAfter;
			...
			if (get_query_var('paged')) {
				...
				$html_markup .= esc_html__('Page', 'roavio') . ' ' . get_query_var('paged');
				if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) {
					$html_markup .= ')';
				}
			}
