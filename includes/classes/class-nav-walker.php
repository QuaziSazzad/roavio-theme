...
class Roavio_Nav_Walker extends Walker_Nav_Menu
{
	...
	public function start_el(&$output, $data_object, $depth = 0, $args = null, $current_object_id = 0)
	{
		...
		$item_meta = get_post_meta($menu_item->ID, 'roavio_nav_menu_meta', true);
	...
	protected function get_additional_classes($menu_item, $item_meta)
	{
		...
		$mega_menu_settings = get_post_meta($menu_item->object_id, 'roavio_tb_settings', true);
	...
	protected function build_link_attributes($menu_item, $args, $depth)
	{
		...
		if ($menu_item->object === 'roavio_template') {
			$mega_menu_url  = get_post_meta($menu_item->ID, '_roavio_mega_menu_url', true);
	...
	protected function get_builder_mega_menu_content($menu_item)
	{
		...
		$settings     = get_post_meta($menu_item->object_id, 'roavio_tb_settings', true);
