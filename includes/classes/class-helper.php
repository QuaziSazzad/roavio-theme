...
class Roavio_Helper
{
	...
	public static function site_layout()
	{
		...
		if (is_page()) {
			$page_layout = self::get_meta('roavio_page_meta', 'site_layout', 'default');
			$layout      = ('default' !== $page_layout) ? $page_layout : $layout;
		} elseif (is_single()) {
			$post_type = get_post_type();

			switch ($post_type) {
				case 'roavio_portfolio':
					$portfolio_layout = self::get_meta('roavio_portfolio_meta', 'portfolio_details_layout', 'default');
					$layout           = ('default' !== $portfolio_layout) ? $portfolio_layout : $layout;
					break;
				case 'product':
					$product_layout = self::get_meta('roavio_product_meta', 'product_details_layout', 'default');
					$layout         = ('default' !== $product_layout) ? $product_layout : $layout;
					break;
				case 'post':
					$post_layout = self::get_meta('roavio_post_meta', 'post_details_layout', 'default');
	...
	public static function content_sidebar()
	{
		...
		if (is_page()) {
			...
			$post_sidebar = self::get_meta('roavio_post_meta', 'post_details_sidebar', 'default');
	...
	public static function container()
	{
		...
		if (is_page() || (is_single() && 'roavio_portfolio' == get_post_type())) {
			if (self::is_elementor_page()) {
				$classes[] = 'container-elementor';
			} else {
				$classes[] = 'container-gap';
			}
		} elseif (is_archive() && 'roavio_portfolio' === get_post_type()) {
	...
	public static function check_default_header()
	{
		...
		if (is_page()) {
			$page_default_header = self::get_meta('roavio_page_meta', 'page_default_header', 'default');
			...
			switch ($post_type) {
				case 'roavio_portfolio':
					$portfolio_default_header = self::get_meta('roavio_portfolio_meta', 'portfolio_default_header', 'default');
					$default_header           = ('default' !== $portfolio_default_header) ? $portfolio_default_header : $default_header;
					break;
				case 'product':
					$product_default_header = self::get_meta('roavio_product_meta', 'product_default_header', 'default');
					$default_header         = ('default' !== $product_default_header) ? $product_default_header : $default_header;
					break;
				case 'post':
					$post_default_header = self::get_meta('roavio_post_meta', 'post_default_header', 'default');
	...
	public static function check_default_footer()
	{
		...
		if (is_page()) {
			$page_default_footer = self::get_meta('roavio_page_meta', 'page_default_footer', 'default');
			...
			switch ($post_type) {
				case 'roavio_portfolio':
					$portfolio_default_footer = self::get_meta('roavio_portfolio_meta', 'portfolio_default_footer', 'default');
					$default_footer           = ('default' !== $portfolio_default_footer) ? $portfolio_default_footer : $default_footer;
					break;
				case 'product':
					$product_default_footer = self::get_meta('roavio_product_meta', 'product_default_footer', 'default');
					$default_footer         = ('default' !== $product_default_footer) ? $product_default_footer : $default_footer;
					break;
				case 'post':
					$post_default_footer = self::get_meta('roavio_post_meta', 'post_default_footer', 'default');
	...
	public static function get_page_colors()
	{
		...
		$is_enabled_page_color = self::get_meta('roavio_page_meta', 'page_custom_color_scheme', 'disabled');
	...
	public static function get_global_fonts()
	{
		$fonts = [
			'_primary'   => [
				'slug'               => 'roavio-base-font',
				'font-family'        => 'Inter',
				'backup-font-family' => 'sans-serif'
			],
			'_secondary' => [
				'slug'               => 'roavio-heading-font',
	...
	public static function get_page_fonts()
	{
		...
		$is_enabled_page_typo   = self::get_meta('roavio_page_meta', 'custom_typo_type', 'default-font');
		...
		$primary_font   = self::get_meta('roavio_page_meta', 'primary_font', []);
		$secondary_font = self::get_meta('roavio_page_meta', 'secondary_font', []);
	...
	public static function site_back_to_top()
	{
		...
		if (is_page()) {
			$back_to_top_page = self::get_meta('roavio_page_meta', 'back_to_top_page', 'default');
